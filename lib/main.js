...
var init = exports.init = function(host) {
    ...
    return {
        ...
        last_build_by_number: function(jobname, number, callback) {
            /*
            Get information for the last build of a job
            */
            request({method: 'GET', url: build_url(BUILD_BY_NUMBER, jobname, number)}, function(error, response, body) {
                if (response.statusCode != 200 || error){
                    callback(error || true, response);
                    return;
                }
